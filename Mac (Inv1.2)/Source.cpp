...
void build(std::string fileName, std::string buildFolder) {
    ...
    std::string fileCopy2 = "cp run.command ./";
...
int main(int argc, char* argv[]) {
    ...
    if(argc > 2) {
        if(argv[2] == std::string("-l")) {
            if(not(Exists("invld"))) {
                std::cout << "ERROR: Can't find the invld program" << std::endl;
            } else {
                std::string entire = "./invld ";
                entire += argv[1];
                entire += " ";
                entire += argv[1];
                entire += "ld > nul";
    ...
    while (getline (MyReadFile, myText)) {
        ...
        if(myText.back() == '.' && not(myText.rfind("\"", 0) == 0)) {
            ...
            // I can't find some thing like getch() on mac :(
            ...
            std::string toVar = split(myText, " -> ")[1];
            ...
            std::string whatVar = split(myText, " + ")[0];
